...
$keepRegex = ".*RDC.*"
$toDisable = Get-WindowsOptionalFeature -Path $MountedImagePath | ?{ ($_.State -eq "Enabled") -and ($_.FeatureName -notmatch $keepRegex) } | %{ $_.FeatureName }
...
foreach ($featureName in $toDisable) {
	...
	Disable-WindowsOptionalFeature -Path $MountedImagePath -FeatureName $featureName
}
